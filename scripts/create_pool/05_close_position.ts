...
async function main() {
  ...
  for (const position of positions) {
    const whirlpool = await client.getPool(position.getData().whirlpool.toBase58());
    ...
    console.log(`Closed the position: ${position.getAddress().toString()} successfully at txid ${txid}`);
  }
}
