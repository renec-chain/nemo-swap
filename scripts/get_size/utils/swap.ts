...
export const getOtherTokenOfPool = (
  pool: Whirlpool,
  baseToken: PublicKey
): PublicKey => {
  if (pool.getData().tokenMintA.toString() == baseToken.toString()) {
    return new PublicKey(pool.getData().tokenMintB);
  } else if (pool.getData().tokenMintB.toString() == baseToken.toString()) {
    return new PublicKey(pool.getData().tokenMintA);
  } else {
