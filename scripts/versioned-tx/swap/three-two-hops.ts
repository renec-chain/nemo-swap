...
async function main() {
  ...
  console.log("wallet:", userAuth.toString());
  ...
  const lookupTableAddressrengnReusd =
    lookupTableData[rengnReusd.getAddress().toBase58()];
  ...
  let lookUpTables = [
    ...
    new PublicKey(lookupTableAddressrenecRengn),
    new PublicKey(lookupTableAddressrengnReusd),
  ];
  ...
  const tx = removeDuplicatedInstructions(
    ...
    new Wallet(userAuth),
    tx1
      .addInstruction(tx2.compressIx(true))
      .addInstruction(tx3.compressIx(true))
  );
