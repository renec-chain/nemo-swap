...
#[derive(Accounts)]
pub struct TwoHopSwapWithFeeDiscount<'info> {
    ...
    /// support one discount token per swap
    #[account(mut)]
    pub discount_token: Account<'info, Mint>,

    #[account(
        seeds = [b"whirlpool_discount_info", whirlpool_one.key().as_ref(), discount_token.key().as_ref()],
        bump,
    )]
    pub whirlpool_discount_info_one: Box<Account<'info, WhirlpoolDiscountInfo>>,

    #[account(
        seeds = [b"whirlpool_discount_info", whirlpool_two.key().as_ref(), discount_token.key().as_ref()],
        bump,
    )]
    pub whirlpool_discount_info_two: Box<Account<'info, WhirlpoolDiscountInfo>>,
