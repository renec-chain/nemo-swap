...
pub fn swap(
    ...
    while amount_remaining > 0 && sqrt_price_limit != curr_sqrt_price {
        ...
        // Return fee, amountIn, amountOut, nextSqrtPrice after a swap step
...
pub fn swap_with_fee_discount(
    ...
    amount: u64,
    sqrt_price_limit: u128,
    amount_specified_is_input: bool, // equal to exact in
    a_to_b: bool,
    timestamp: u64,
) -> Result<(PostSwapUpdate, u64, u64), ErrorCode> {
    if sqrt_price_limit < MIN_SQRT_PRICE_X64 || sqrt_price_limit > MAX_SQRT_PRICE_X64 {
        return Err(ErrorCode::SqrtPriceOutOfBounds.into());
    }
    ...
    while amount_remaining > 0 && sqrt_price_limit != curr_sqrt_price {
        ...
        // Return fee, amountIn, amountOut, nextSqrtPrice after a swap step
        let mut swap_computation = compute_swap(
            amount_remaining,
            fee_rate,
            curr_liquidity,
            curr_sqrt_price,
